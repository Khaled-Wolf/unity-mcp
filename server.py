...
@mcp.tool()
async def connect_to_llm(ctx: Context, llm_name: str) -> str:
    if llm_name == "claude":
        await _connect_to_claude(ctx)
    elif llm_name == "aider":
        await _connect_to_aider(ctx)
    else:
        return f"LLM client {llm_name} not supported."
    
    return f"Connected to LLM client: {llm_name}"

async def _connect_to_claude(ctx: Context) -> None:
    # Implementation for connecting to Claude
    pass

async def _connect_to_aider(ctx: Context) -> None:
    # Implementation for connecting to Aider
    pass
